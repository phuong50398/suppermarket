                ...
                <div class="panel panel-primary">
                    ...
                    <div class="panel-body">
                        <form action="{{(isset($action)) ? route('category.update', $category->id) : route('category.store')}}" method="POST" class="form-horizontal">
                            ...
                            <div class="form-group row">
                                <label for="fname" class="col-sm-3 text-right control-label col-form-label">Nhóm danh mục</label>
                                <div class="col-sm-9">
                                    <select name="categoryGroup" class="form-control">
                                        @if (isset($action))
                                            @foreach ($listCategoryGroup as $cg)
                                                <option {{(old('category_group_id', $category->category_group_id)==$cg->id) ? "selected" : ""}} value="{{$cg->id}}">{{$cg->name}}</option>
                                            @endforeach
                                        @else
                                            @foreach ($listCategoryGroup as $cg)
                            ...
                            <tbody>
                                @foreach ($listCategory as $key => $item)
                                    <tr>
                                        <td>{{$key+1}}</td>
                                        <td>{{$item->name}}</td>
                                        <td>{{$arrcg[$item->category_group_id]}}</td>
                                        ...
                                        <td>
                                            ...
                                            <form action="{{route('category.destroy', $item->id)}}" method="POST" style="display: contents;">
                                                @method('DELETE')
                                                @csrf
                                                <button type="submit" class="btn btn-danger btn-sm"" onclick="return confirm('Are you sure?')" {{(count($item->categoryType)>0) ? 'disabled' : ""}} title="Xóa"><i class=" fas fa-trash-alt"></i></button>
                                            </form>
                                        </td>
                                    </tr>
                                @endforeach
