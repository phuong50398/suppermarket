        ...
        <div class="col-md-12 col-lg-12 col-xlg-12 p-t-30 row p-r-50 p-l-50">
            <form action="{{(isset($action)) ? route('billExport.update', $billExport->id) : route('billExport.store')}}" method="POST" class=" w-100">
                ...
                <div class="col-md-12 text-right m-t-30 p-r-50">
                    @if (isset($action))
                    <button type="submit" class="btn btn-info" name="status" value="0" {{($billExport->status==1) ? "disabled" : ""}}><i class=" fas fa-save" aria-hidden="true"></i> Cập nhật đơn xuất hàng</button>
                    <button type="submit" class="btn btn-success" name="status" value="1" {{($billExport->status==1) ? "disabled" : ""}}><i class="fas fa-check" aria-hidden="true"></i> Cập nhật đơn xuất hàng và Xác nhận xuất xong</button>
                    @else
                    <button type="submit" class="btn btn-info" name="status" value="0"><i class=" fas fa-save" aria-hidden="true"></i> Tạo đơn xuất hàng</button>
                    <button type="submit" class="btn btn-success" name="status" value="1"><i class="fas fa-check" aria-hidden="true"></i> Tạo đơn xuất hàng và Xác nhận xuất hàng xong</button>
                    @endif
                </div>
            </form>
        </div>
